...
public class ScoreboardManager {
    ...
    public ScoreboardManager() {
        ...
        glowingTask = Shootcraft.getInstance().getScheduledExecutorService().scheduleAtFixedRate(() ->
        {
            String ip = colorIpAt();
            for (PersonalScoreboard scoreboard : scoreboards.values())
                Shootcraft.getInstance().getExecutorMonoThread().execute(() -> scoreboard.setLines(ip));
        }, 80, 80, TimeUnit.MILLISECONDS);

        reloadingTask = Shootcraft.getInstance().getScheduledExecutorService().scheduleAtFixedRate(() ->
        {
            for (PersonalScoreboard scoreboard : scoreboards.values())
                Shootcraft.getInstance().getExecutorMonoThread().execute(scoreboard::reloadData);
